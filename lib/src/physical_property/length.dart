...
class Length extends PhysicalProperty {
  ...
  Length._internal(String kind) : super(kind: kind) {
    ...
    _internationalSystemOfUnits.defineUnitWithFactor(symbol: 'pm', name: 'picometer', factor: 0.000000000001);
    _internationalSystemOfUnits.defineUnitWithFactor(symbol: 'nm', name: 'nanomiter', factor: 0.000000001);
    _internationalSystemOfUnits.defineUnitWithFactor(symbol: 'micro', name: 'micrometer', factor: 0.000001);
    _internationalSystemOfUnits.defineUnitWithFactor(symbol: 'mm', name: 'millimeter', factor: 0.001);
    _internationalSystemOfUnits.defineUnitWithFactor(symbol: 'cm', name: 'centimeter', factor: 0.01);
    _internationalSystemOfUnits.defineUnitWithFactor(symbol: 'dm', name: 'decimeter', factor: 0.1);
    _internationalSystemOfUnits.defineUnitWithFactor(symbol: 'km', name: 'kilometer', factor: 1000.0);
    ...
    systemsOfMeasurent.addAll([_internationalSystemOfUnits, _imperialSystemOfUnits, _nauticalSystemOfUnits]);
  }
  ...
  static Quantity millimeters({required double amount}) => Quantity(unit: mm, amount: amount);
  static Quantity centimeters({required double amount}) => Quantity(unit: cm, amount: amount);
  static Quantity decimeters({required double amount}) => Quantity(unit: dm, amount: amount);
  static Quantity meters({required double amount}) => Quantity(unit: m, amount: amount);
  static Quantity kilometers({required double amount}) => Quantity(unit: km, amount: amount);
