...
class Volume extends PhysicalProperty {
  ...
  Volume._internal(String kind) : super(kind: kind) {
    ...
    _internationalSystemOfUnits.defineUnitWithFactor(symbol: 'mm3', name: 'cubic millimeter', factor: 0.000001);
    _internationalSystemOfUnits.defineUnitWithFactor(symbol: 'cm3', name: 'cubic centimeter', factor: 0.001);
    _internationalSystemOfUnits.defineUnitWithFactor(symbol: 'dm3', name: 'cubic decimeter', factor: 1.0);
    _internationalSystemOfUnits.defineUnitWithFactor(symbol: 'm3', name: 'cubic meter', factor: 1000.0);
    ...
    _imperialSystemOfUnits.defineUnitWithFactor(symbol: 'fl oz', name: 'fluid ounce', factor: 1.0 / 160.0);
  ...
  static Quantity cubicMillimimeters({required double amount}) => Quantity(unit: mm3, amount: amount);
  static Quantity cubicCentimeters({required double amount}) => Quantity(unit: cm3, amount: amount);
  static Quantity cubicDecimimeters({required double amount}) => Quantity(unit: dm3, amount: amount);
  static Quantity cubicMeters({required double amount}) => Quantity(unit: m3, amount: amount);
  static Quantity liters({required double amount}) => Quantity(unit: l, amount: amount);
